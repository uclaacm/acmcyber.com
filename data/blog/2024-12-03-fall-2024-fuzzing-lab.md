...
[PoDoFo](https://github.com/podofo/podofo) is an open-source C++17 library to manipulate PDF files. It allows PDF files to be edited, read from, and signed. We fuzzed a set of functions from the library, including `LoadFromBuffer()`, `ExtractTextTo()`, and a variety of `Get*()` functions (for getting the properties and contents of a PDF file). 
