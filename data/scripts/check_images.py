...
with open("data/scripts/tmp/old-member-data", "r") as f:
    old_data = f.read()
old_data_json = json.loads(old_data)
with open("data/scripts/clubdata/membership.tsv", "r") as f:
    new_extra_data = list(
        filter(
            lambda x: "Not found" not in x,
            [OtherInfo(*i.split("\t")[:22]) for i in f.read().split("\n")[1:]],
        )
    )
members = []
